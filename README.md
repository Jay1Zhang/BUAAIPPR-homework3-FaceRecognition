...
从上述实验结果中可以看出，特征脸方法最耗时，线性判别分析法次之，LBPH方法速度最快。
...
从上述实验结果中可以看出，在训练轮数相同的情况下，不同姿态对于识别率的影响并不大。
...
从实验结果中可以看出，对于本数据集来说，经典算法与基于CNN的方法识别率相差无几。除了这几种算法本身的优异性之外，还可能是由于该数据集较为简单。对于更大型的或者场景更为复杂的人脸识别数据库中，基于CNN的算法应该会有更加优异的表现。
